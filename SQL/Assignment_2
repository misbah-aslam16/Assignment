...
--Q1	Show all product names along with their brand name. Sort by brand name, then by product name alphabetically.	INNER JOIN
...
--Q2	List all products with their category name and list price. Sort by category name, then by price from cheapest to most expensive.	INNER JOIN
...
--Q3	Show all orders with the customer's full name and order date. Sort by order date from newest to oldest.	INNER JOIN
...
--Q4	Display each order item with the product name, quantity, unit price, and a computed column called "Line Total" (quantity × list_price). Sort by order ID.	INNER JOIN
...
--Q5	Show each order along with the store name where it was placed and the order date. Sort by store name.	INNER JOIN
...
--Q6	Show each order with: order ID, customer full name, store name, and the staff member's full name who handled it.	INNER JOIN
...
--Q7	List all products from the brand "Trek" along with their category name and price. Sort by price descending. (Use JOIN — do NOT filter by brand_id directly.)	INNER JOIN
...
--Q8	Find all customers from the state of "NY" who have placed at least one order. Show customer full name, city, and order date. (Use JOIN — do not use a subquery.)	INNER JOIN
...
--Q9	Show all completed orders (order_status = 4) from the store "Rowlett Bikes". Display order ID, customer full name, and order date.	INNER JOIN

select
o.order_id,c.first_name+ ' '+ c.last_name as full_name,
o.order_date,
s.store_name,
o.order_status 
from 
...
--Q10	List ALL customers and any orders they have placed. Include customers who have never placed an order (show NULL for order columns). Sort by customer ID.	LEFT JOIN
...
--Q11	Find all customers who have NEVER placed an order. Show their full name and email.	LEFT JOIN
